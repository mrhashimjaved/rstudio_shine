...
#ease_df <- read.spss("2. EASE (n=5233) (PSC Nvalid=35).sav", to.data.frame = TRUE)
#ease_df <- read.spss("2. EASE (n=5856).sav", to.data.frame = TRUE)
ease_psc <- read.spss("2. EASE (n=5233) (PSC Nvalid=35).sav", to.data.frame = TRUE)
ease_sdq <- read.spss("2. EASE (n=5222) (SDQ Nvalid=25).sav", to.data.frame = TRUE)

#selecting PSC variables for analysis
psc_df1 <- ease_psc[,c(14:48,93:94)]
sdq_df1 <- ease_sdq[,c(49:73,87:92)]
...
library(psych)
poly_values = polychoric(psc_df2)
polyMat <- poly_values$rho

library(qgraph)
#partial correlation matrix
psc_corMat <- cor_auto(psc_df2)

#Unregularized partial correlation netwrok
psc_pcor <- qgraph(cor(psc_df2), layout="spring", tuning=0.25, groups=psc_gps,
                       legend.cex=0.50, vsize=4, esize=15)

#Regularized partial correlation Network
psc_Glasso <- qgraph(psc_corMat, graph="glasso", layout="spring", tuning=0.25, 
                  sampleSize=nrow(psc_df2), groups=psc_gps, nodeNames=psc_items, 
                  legend.cex=0.20, vsize=4, esize=15, minimum=0.075)

#checking for redundant nodes
library(networktools)
goldbricker(psc_df2,p=0.05)
...
psc_Glasso <- qgraph(psc_corMat, graph="glasso", layout="spring", tuning=0.25, sampleSize=nrow(psc_df2))
...
psc_net <- estimateNetwork(psc_df2, default="EBICglasso")
...
psc_boot1 <- bootnet(psc_net, nBoots=2500, nCores=8)
plot(psc_boot1, labels=FALSE, order="sample")

#iii. Network stability (Using bootstrapping)
psc_boot2 <- bootnet(psc_net, nBoots=2500, type="case", nCores=8)
plot(psc_boot2)
corStability(psc_boot2)

#iv. Testing for significance differences
differenceTest(psc_boot1, 22,33,"strength")

  #testing between all pairs 
  #Warning!!
            #Requires a lot of RAM !!

plot(psc_boot1, "edge", plot="difference", onlyNonZero=TRUE, order="sample")
...
layout(t(1:2))
psc_pcor <- qgraph(psc_corMat, graph="psc_pcor", layout="spring")
psc_Glasso <- qgraph(psc_corMat, graph="glasso", layout="spring", tuning=0.25, sampleSize=nrow(psc_df2))
...
plot(eigen(psc_corMat)$values, type = "b")
abline(h=1, col="red", lty=3)

# SPINGLASS ALGORITHM
library(igraph)
g = as.igraph(psc_Glasso, attributes=TRUE)
...
sdq_df1 <- ease_df[,c(12,54:98)]
sdq_df2 <- sdq_df1[,2:26]
#library(tidyverse)
